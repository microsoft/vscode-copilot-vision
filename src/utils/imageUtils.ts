...
// this is same as above, but gets entire range of the image syntax
export function extractImageAttributes2(line: string, refineExisting?: boolean): { imagePath: string, altTextStartIndex: number, isHTML: boolean, altTextLength: number, range: { start: number, end: number } } | undefined {
	// Regex to match markdown image syntax ![alt text](image_path)
	const markdownImageRegex = /!\[([^\]]*)\]\(([^)]+)\)/;
	// Updated regex to match HTML image syntax with alt and src in any order
	const htmlImageRegex = /<img\s+(?:alt=["']([^"']*)["']\s*)?src=["']([^"']+)["'](?:\s*alt=["']([^"']*)["'])?/;
	...
	if ((match = markdownImageRegex.exec(line)) !== null) {
		...
		return { imagePath, altTextStartIndex, isHTML, altTextLength, range };
	}
